...
const TestimonialLoopSection = () => {
  ...
  useEffect(() => {
    ...
    leftCardsArray.forEach((card) => leftRowRef.current.appendChild(card.cloneNode(true)));
    rightCardsArray.forEach((card) => rightRowRef.current.appendChild(card.cloneNode(true)));

    // Calculate total width of cards in each row
...
export default TestimonialLoopSection;
